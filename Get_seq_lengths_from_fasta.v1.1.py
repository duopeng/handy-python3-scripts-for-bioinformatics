...
def main():
	try:
		with open(input_file, "rU") as handle, open("{}.seqlengths".format(input_file), "w") as writehandle: # rU means open for reading using universal readline mode this means you dont have to worry if the file uses Unix, Mac or DOS Windows style newline characters The with- statement makes sure that the file is properly closed after reading it
